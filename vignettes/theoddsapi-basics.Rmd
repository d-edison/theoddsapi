...
As a "hello world" to confirm that everything is configured correctly, there is function `get_remaining_requests` that informs you how many requests are remaining:
...
The principal function of this package, `get_odds` allows you to request odds for any of the sports returned by the `get_sports` function. If called without any arguments, it returns the odds for any live contest as well as the 8 upcoming games across all sports:

```{r}
get_odds()
```

`get_odds` always returns a tibble with columns sport, commence_time, team, book, and whatever is set for the `mkt` argument (by default, "h2h").
...
nba_spreads <- get_odds(
    "basketball_nba", # key field from get_sports
    mkt = "spreads" # point spread
)
...
I also welcome anyone to file an issue or submit a pull request on GitHub if you encounter an issue or have a suggestion for improvement!
